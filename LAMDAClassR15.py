...
datos=pd.read_csv('C:/Doctorado/Data/R15.csv',  header=0, delimiter=';', usecols=range(0,3))#columnas del dataset
...
for i in range(0, clases):
    for j in range(0, len(dfnorm)):
        dfnorm.loc[j,'MaximoC'+repr(i)]=max(dfnorm.loc[j,'MAD'+repr(i)+'0'],dfnorm.loc[j,'MAD'+repr(i)+'1'])
        dfnorm.loc[j,'MinimoC'+repr(i)]=min(dfnorm.loc[j,'MAD'+repr(i)+'0'],dfnorm.loc[j,'MAD'+repr(i)+'1'])
        dfnorm.loc[j,'GAD'+repr(i)]=alfa*dfnorm.loc[j,'MinimoC'+repr(i)]+(1-alfa)*dfnorm.loc[j,'MaximoC'+repr(i)]
for i in range(0,len(dfnorm)):
    dfnorm.loc[i,'index']=max(dfnorm.loc[i,'GAD0'],dfnorm.loc[i,'GAD1'], dfnorm.loc[i,'GAD2'], dfnorm.loc[i,'GAD3'], dfnorm.loc[i,'GAD4'], dfnorm.loc[i,'GAD5'], dfnorm.loc[i,'GAD6'], dfnorm.loc[i,'GAD7'], dfnorm.loc[i,'GAD8'], dfnorm.loc[i,'GAD9'], dfnorm.loc[i,'GAD10'], dfnorm.loc[i,'GAD11'], dfnorm.loc[i,'GAD12'], dfnorm.loc[i,'GAD13'], dfnorm.loc[i,'GAD14'],  dfnorm.loc[i,'GADNIC'])
    ...
    elif (dfnorm.loc[i,'index']==dfnorm.loc[i,'GAD2']):
        dfnorm.loc[i,'clase1']=3
    elif (dfnorm.loc[i,'index']==dfnorm.loc[i,'GAD3']):
        dfnorm.loc[i,'clase1']=4
    elif (dfnorm.loc[i,'index']==dfnorm.loc[i,'GAD4']):
        dfnorm.loc[i,'clase1']=5
    elif (dfnorm.loc[i,'index']==dfnorm.loc[i,'GAD5']):
        dfnorm.loc[i,'clase1']=6
    elif (dfnorm.loc[i,'index']==dfnorm.loc[i,'GAD6']):
        dfnorm.loc[i,'clase1']=7
    elif (dfnorm.loc[i,'index']==dfnorm.loc[i,'GAD7']):
        dfnorm.loc[i,'clase1']=8
    elif (dfnorm.loc[i,'index']==dfnorm.loc[i,'GAD8']):
        dfnorm.loc[i,'clase1']=9
    elif (dfnorm.loc[i,'index']==dfnorm.loc[i,'GAD9']):
        dfnorm.loc[i,'clase1']=10
    elif (dfnorm.loc[i,'index']==dfnorm.loc[i,'GAD11']):
        dfnorm.loc[i,'clase1']=12
    elif (dfnorm.loc[i,'index']==dfnorm.loc[i,'GAD12']):
        dfnorm.loc[i,'clase1']=13
    elif (dfnorm.loc[i,'index']==dfnorm.loc[i,'GAD13']):
        dfnorm.loc[i,'clase1']=14
    else:
        dfnorm.loc[i,'clase1']=15
ruta='C:\Python\doctorado\LAMDAR15Clasificacion.csv'#EXPORTACIÓN DE LOS RESULTADOS
dfnorm.to_csv(ruta)
#print('este es el ultimo dataaset')

dflisto= dfnorm.iloc[:, [1, 2, 0, 80]]#Este DataFrame esta listo para hacer grafico y bondad de ajuste
print(dflisto)
...
dfnormorig=dfnormorig.drop(['0' , '1'], axis=1)#borro la columna de descriptor no estandarizados
...
pca_R15=pca.fit_transform(dfnormorig) #Contiene las coordenadas de los individuos esto implica que se hace la transformación lineal
pca_R15LAMDA_df=pd.DataFrame(data=pca_R15, columns= ['Componente_1', 'Componente_2']) #creación del data frame con las coordenadas de los dos componentes 
pca_nombres_R15=pd.concat([pca_R15LAMDA_df, dfnorm[['clases']]], axis=1)#Agrego la columna de la etiqueta
print(pca_R15LAMDA_df)
print(pca_nombres_R15)
fig=plt.figure(figsize=(10,10))
ax=fig.add_subplot(1,1,1)
ax.set_xlabel('Componente 1', fontsize=15)
ax.set_ylabel('Componente 2', fontsize=15)
ax.set_title('Data R15 Original', fontsize=20)
color_theme= np.array(['red','blue','orange','black','pink','green','brown','purple','yellow','gray','black','blue','red','orange','purple','brown'])
ax.scatter(x=pca_R15LAMDA_df.Componente_1 , y= pca_R15LAMDA_df.Componente_2, c=color_theme[pca_nombres_R15.clases], s= 10, marker='.')
plt.show()
